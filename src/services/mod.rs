...
pub fn get_user_by_id(conn: &PgConnection, user_id: Uuid) -> QueryResult<User> {
    users.filter(id.eq(user_id)).first(conn)
}

pub fn update_user(conn: &PgConnection, user_id: Uuid, updated_user: NewUser) -> QueryResult<User> {
